...
export default function HeroImageManagerModal({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (onClose) {
        ...
        alert("Lưu cài đặt ảnh thành công!");
      }
    } catch (error: any) {
      console.error("Error saving hero images:", error);
      alert(`Đã xảy ra lỗi khi lưu: ${error.message || "Lỗi không xác định"}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const content = (
    <div
      className={
        inline
          ? "w-full p-6 flex flex-col max-w-2xl mx-auto"
          : "bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg p-6 shadow-xl border border-slate-200 dark:border-slate-800"
      }
    >
