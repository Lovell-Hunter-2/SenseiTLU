    ...
    <>
      ...
      <div className={isMobileView ? "flex md:hidden justify-between items-stretch gap-1 mb-2 font-mono w-full text-[9px] min-[375px]:text-[10px] min-[400px]:text-xs z-20 relative" : "hidden md:flex items-center gap-2 mr-2 font-mono"}>
         <div className={isMobileView 
            ...
            : "flex items-center gap-1.5 bg-gradient-to-r from-pink-100 to-rose-100 dark:from-pink-900/40 dark:to-rose-900/40 px-3.5 py-1.5 rounded-2xl border border-pink-200 dark:border-pink-800/50 shadow-sm text-pink-700 dark:text-pink-300 font-bold transition-transform hover:scale-105 cursor-default relative group overflow-hidden"}>
            <span className="relative flex items-center justify-center w-4 h-4 min-[375px]:w-5 min-[375px]:h-5 md:w-5 md:h-5 overflow-visible z-10">
              <span className={`z-10 relative ${isMobileView ? 'text-sm min-[375px]:text-base flower-icon' : 'text-lg group-hover:animate-bounce'}`}>🌸</span>
            </span>
            ...
            <span className={`z-10 relative ${isMobileView ? 'whitespace-nowrap tracking-tighter min-[375px]:tracking-normal' : 'ml-1 text-base tracking-wider'}`}>{formatTime(now)}</span>
            <span className={`opacity-60 animate-pulse z-10 relative ${isMobileView ? 'text-[8px] min-[375px]:text-[10px] hidden min-[375px]:inline' : 'text-[10px] ml-0.5'}`}>{getSeconds(now)}</span>
         </div>
         ...
         {isHome && (
           <>
             <div className={isMobileView 
                ...
                : "hidden lg:flex items-center gap-2 bg-gradient-to-r from-cyan-100 to-blue-100 dark:from-cyan-900/40 dark:to-blue-900/40 px-3.5 py-1.5 rounded-2xl border border-cyan-200 dark:border-cyan-800/50 shadow-sm text-cyan-700 dark:text-cyan-300 font-bold transition-transform hover:scale-105 cursor-default group"}>
                <span className={`relative flex items-center justify-center w-5 h-5 min-[375px]:w-6 min-[375px]:h-6 md:w-5 md:h-5 overflow-visible origin-bottom ${isMobileView ? '' : 'group-hover:animate-sway'}`}>
                  <span className={`absolute z-10 ${isMobileView ? 'text-sm min-[375px]:text-base transition-transform duration-500 cloud-icon-inner' : 'text-[18px] transition-transform duration-500 group-hover:-translate-x-1.5'}`}>☁️</span>
                  <span className={`absolute z-0 opacity-0 scale-50 -translate-x-2 translate-y-1 transition-all duration-500 ease-out ${isMobileView ? 'text-xs min-[375px]:text-sm sun-icon' : 'text-base group-hover:opacity-100 group-hover:scale-100 group-hover:translate-x-2 group-hover:-translate-y-1 group-hover:rotate-12'}`}>☀️</span>
                </span>
                <span className={isMobileView ? "whitespace-nowrap tracking-tighter min-[375px]:tracking-normal" : "text-base tracking-wider"}>{formatDate(now)}</span>
             </div>
             ...
             <div className={isMobileView
                ...
                : "flex items-center gap-2 bg-gradient-to-r from-amber-100 to-orange-100 dark:from-amber-900/40 dark:to-orange-900/40 px-3.5 py-1.5 rounded-2xl border border-amber-200 dark:border-amber-800/50 shadow-sm text-amber-700 dark:text-amber-300 font-bold transition-transform hover:scale-105 cursor-default group"} title="Thời gian học hôm nay">
                <span className="relative flex items-center justify-center w-4 h-4 min-[375px]:w-5 min-[375px]:h-5 md:w-5 md:h-5 overflow-visible">
                  <span className={`absolute transition-all duration-700 ease-in-out transform ${isMobileView ? 'text-sm min-[375px]:text-base seed-icon' : 'text-[18px] group-hover:scale-0 group-hover:opacity-0 group-hover:-translate-y-2'}`}>🌱</span>
                  <span className={`absolute transition-all duration-700 ease-in-out transform scale-0 opacity-0 translate-y-2 ${isMobileView ? 'text-sm min-[375px]:text-base tree-icon' : 'text-[20px] group-hover:scale-125 group-hover:opacity-100 group-hover:translate-y-0'}`}>🌳</span>
                </span>
                <span className={isMobileView ? "whitespace-nowrap tracking-tighter min-[375px]:tracking-normal" : "text-base tracking-wider"}>{formatSpent(timeSpent)}</span>
             </div>
           </>
         )}
      </div>
